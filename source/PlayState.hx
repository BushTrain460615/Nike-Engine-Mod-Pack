...
class PlayState extends MusicBeatState
{
	...
	//healthbar shit lol
	var OpponentHealthbar:Dynamic = 0xFFFF0000;
	var BFHealthbar:Dynamic = FlxColor.fromRGB(0, 185, 220);
	var oldIcon:Bool = false;

	//score
	var PixelSONG:Bool = false;
	var week7zoom:Bool = false;
	...
	// this doesn't need to be only for html5 lol
	...
	override public function create()
	{
		...
		OpponentHealthbar = 0xFFFF0000;
		BFHealthbar = FlxColor.fromRGB(0, 185, 220);
		oldIcon = false;
		...
		if (FileSystem.exists('assets/data/' + SONG.song.toLowerCase() + '/stage/script.hx')) {
			stageMOD = true;
		}
		...
		var path:String = Paths.hscript(formattedFolder + '/' + SONG.song.toLowerCase());
		...
		switch (curStage)
		{
			...
			case 'algebra': {
				defaultCamZoom = 0.85;
				songSpeed = 1.6;
				var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('mods/dave/algebraBg'));
				...
				daveJunk = new FlxSprite(424, 122).loadGraphic(Paths.image('mods/dave/bgJunkers/dave'));
				davePiss = new FlxSprite(427, 94);
				davePiss.frames = Paths.getSparrowAtlas('mods/dave/bgJunkers/davePiss');
				...
				garrettJunk = new FlxSprite(237, 59).loadGraphic(Paths.image('mods/dave/bgJunkers/bitch'));
				garrettJunk.y += 45;

				monitorJunk = new FlxSprite(960, 61).loadGraphic(Paths.image('mods/dave/bgJunkers/rubyIsAngryRN'));
				monitorJunk.x += 275;
				monitorJunk.y += 75;

				diamondJunk = new FlxSprite(645, -16).loadGraphic(Paths.image('mods/dave/bgJunkers/lanceyIsGoingToMakeAFakeLeakAndPostItInGeneral'));
				diamondJunk.x += 75;

				robotJunk = new FlxSprite(-160, 225).loadGraphic(Paths.image('mods/dave/bgJunkers/myInternetJustWentOut'));
				robotJunk.x -= 250;
				robotJunk.y += 75;
            ...
            case 'void': {
				...
				void.frames = Paths.getSparrowAtlas('mods/agoti/The_void', 'shared');
				...
				bgRocks = new FlxSprite(-1000, -700).loadGraphic(Paths.image('mods/agoti/Void_Back', 'shared'));
				bgRocks.setGraphicSize(Std.int(bgRocks.width * 0.5));
				bgRocks.antialiasing = true;
				bgRocks.scrollFactor.set(0.7, 0.7);
				add(bgRocks);

				var frontRocks:FlxSprite = new FlxSprite(-1000, -600).loadGraphic(Paths.image('mods/agoti/Void_Front', 'shared'));
			...
			case 'too-slow':
				...
				treesFar = new FlxSprite(0, 0).loadGraphic(Paths.image('mods/PolishedP1/TreesMid', 'shared'));
				...
				treesFarthest = new FlxSprite(0, 0).loadGraphic(Paths.image('mods/PolishedP1/TreesMidBack', 'shared'));
		...
		SicksText = new FlxText(0, 335, "", 24);
		...
		GoodsText = new FlxText(0, 370, "", 24);
		...
		ShitsText = new FlxText(0, 440, "", 24);
		...
		imBoutaDie = new FlxSprite(0, 0).loadGraphic(Paths.image('VG'));
		imBoutaDie.antialiasing = Settings.Antialiasing;
		imBoutaDie.cameras = [camOTHER];
		imBoutaDie.screenCenter();
		imBoutaDie.alpha = 0;
		add(imBoutaDie);

		if (storyDifficultyText.toLowerCase() != 'erect') {
			imBoutaDie.color = FlxColor.RED;
		} else {
			imBoutaDie.color = FlxColor.PURPLE;
		}
	...
	function getDialogue(song:String) {
		dialogue = CoolUtil.coolTextFile(Paths.txt('$song/$song' + 'Dialogue'));

		doof = new DialogueBox(false, dialogue); //re-add dialogue lol!
	...
	function playCutscene(name:String, atEndOfSong:Bool = false, ?isMidSongCutscene:Bool = false)
	{
		...
		{	
			if (scriptStuff()) {
				script.executeFunc("onCutsceneComplete");
			}
	...
	function scriptStuff():Bool {
		if (script != null) {
			return true;
		} else {
			// trace('Script is null!'); literally spammed this in terminal
			return false;
		}
	}
	...
	function ChangeCHAR(Player:Int, X:Dynamic, Y:Dynamic, NewCHAR:Dynamic, ?HideFlash:Bool = false) {
		...
		if (scriptStuff()) {
			script.executeFunc("onCharChange");
		}
	}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				default:
					if (curStage != 'algebra' && curStage != 'garrett-school') {
	...
	private var LoadingSPAM:Bool = false;
	var NOTESPAM:Bool = false;

	var BackgroundSPEED:Float = 15;

	var canPlayimBoutaDie:Bool = true;
	var imBoutaDietimer:Float = 0.75;
	...
	override public function update(elapsed:Float)
	{
		...
		*/
		...
		if (!LoadingScreen.caching && startedCountdown && canPause && !LoadingSPAM) {
			...
			// LoadingScreen.caching = false; //loading sucks
		}
		...
		*/
		...
		if (healthBar.percent < 20) {
			...
			if (canPlayimBoutaDie) {
				if (camHUD.alpha == 0.6 && camOTHER.alpha == 0.6 && imBoutaDie.alpha == 0.6) {
					canPlayimBoutaDie = false;

					FlxTween.tween(camHUD, {alpha: 1}, imBoutaDietimer, {ease: FlxEase.quadInOut});
					FlxTween.tween(camOTHER, {alpha: 1}, imBoutaDietimer, {ease: FlxEase.quadInOut});
					FlxTween.tween(imBoutaDie, {alpha: 1}, imBoutaDietimer, {ease: FlxEase.quadInOut,
						onComplete: function(flx:FlxTween) {
							canPlayimBoutaDie = true;
						}
					});
				} else if (camHUD.alpha == 1 && camOTHER.alpha == 1 && imBoutaDie.alpha == 1) {
					canPlayimBoutaDie = false;

					FlxTween.tween(camHUD, {alpha: 0.6}, imBoutaDietimer, {ease: FlxEase.quadInOut});
					FlxTween.tween(camOTHER, {alpha: 0.6}, imBoutaDietimer, {ease: FlxEase.quadInOut});
					FlxTween.tween(imBoutaDie, {alpha: 0.6}, imBoutaDietimer, {ease: FlxEase.quadInOut,
						onComplete: function(flx:FlxTween) {
							canPlayimBoutaDie = true;
						}
					});
				} else if (camHUD.alpha == 1 && camOTHER.alpha == 1 && imBoutaDie.alpha == 0) {
					canPlayimBoutaDie = false;
		...
		if (healthBar.percent < 79 && healthBar.percent > 19) {
			if (camHUD.alpha == 0.6 && camOTHER.alpha == 0.6) {
				FlxTween.tween(camHUD, {alpha: 1}, imBoutaDietimer, {ease: FlxEase.quadInOut});
				FlxTween.tween(camOTHER, {alpha: 1}, imBoutaDietimer, {ease: FlxEase.quadInOut});
			}
			
			if (imBoutaDie.alpha == 0.6 || imBoutaDie.alpha == 1) {
				FlxTween.tween(imBoutaDie, {alpha: 0}, imBoutaDietimer, {ease: FlxEase.quadInOut});
			}
		}
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					{
						case 0:
							dad.playAnim('singLEFT' + dadAltAnim, true);
							/*
						...
						case 1:
							dad.playAnim('singDOWN' + dadAltAnim, true);
							/*
							if (!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && !daNote.isSustainNote) {
								CamMovement = FlxTween.tween(camFollow, {x: dad.x, y: dad.y +offset}, Conductor.crochet / 10000, {ease: FlxEase.quadInOut});
							} */
						case 2:
							dad.playAnim('singUP' + dadAltAnim, true);
							/*
							if (!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && !daNote.isSustainNote) {
								CamMovement = FlxTween.tween(camFollow, {x: dad.x, y: dad.y -offset}, Conductor.crochet / 10000, {ease: FlxEase.quadInOut});
							} */
						case 3:
							dad.playAnim('singRIGHT' + dadAltAnim, true);
							/*
							if (!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && !daNote.isSustainNote) {
								CamMovement = FlxTween.tween(camFollow, {x: dad.x +offset, y: dad.y}, Conductor.crochet / 10000, {ease: FlxEase.quadInOut});
							} */
					...
					if (dad.animation.curAnim.name.contains('-alt') && dadAltAnim == '-alt') {
						dadAltAnim = '';
					}
					...
					var damage:Float;
					var holdDamage:Float;

					switch (dad.curCharacter.toLowerCase()) {
						case 'og-dave-angey' | 'pedophile' | 'garrett-car' | 'sonicexe':
							damage = 0.018;
							holdDamage = 0.003;
						default:
							damage = 0.0165;
	...
	function OpponentLetNoteGO():Void {
		var shits:String = dad.animation.curAnim.name + dadAltAnim;
		opponentStrums.forEach(function(spr:FlxSprite)
		{
	...
	function NormalMiss() {
		if (Main.framerate > 30) { //no misses due to lag
			combo = 0; //you lose your combo and miss when you don't hit a note lol
			misses += 1;
			health -= 0.0475;
			vocals.volume = 0;
		} else {
			trace('No miss due to lag.');
		}

		if (scriptStuff()) {
			script.executeFunc("onMiss");
		}
	}
	...
	*/
	...
	private function popUpScore(strumtime:Float, note:Note):Void
	{
		...
		else if (noteDiff > Conductor.safeZoneOffset * 0.2)
		{
			...
			sicks += 1;
			health += 0.0125; //little reward for hitting sicks
			daRating = "sick";
		...
		if (SONG.song == 'Ferocious' || SONG.song == 'Algebra') {
	...
	private function keyShit():Void
	{
		...
		if (!controlArray.contains(true) && !holdingArray.contains(true) && releaseArray.contains(true)) {
			boyfriend.holdTimer = 0.005;
		} else if (controlArray.contains(true) || holdingArray.contains(true)) {
			boyfriend.holdTimer -= 1; // to make sure idle doesn't play in anims lol!
		}
		...
		if (boyfriend.holdTimer > 0.004 * Conductor.stepCrochet && !holdingArray.contains(true) && boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss') #if mods
			&& !boyfriend.animation.curAnim.name.contains("first") && !boyfriend.animation.curAnim.name.contains("taunt") #end)
		{
			boyfriend.playAnim('idle');
		}
	...
	function TankmanSHITS(CAMZOOM:Bool, ?ALPHA:Float = 1) {
		if (CAMZOOM) {
			week7zoom = true;
	...
	public function startScript()
	{
		...
		if (hxdata != "")
		{
			...
			script.setVariable("onDeath", function()
			{
			});

			script.setVariable("onSongEnd", function()
			{
			});
			...
			script.setVariable("onMiss", function()
			{
			});

			script.setVariable("onCutsceneComplete", function()
			{
			});

			script.setVariable("onCharChange", function()
			{
			});
			...
			script.setVariable("Settings", Settings);
	...
	function applicationPopup(Title:String, Message:String) {
		...
		if (SONG.song.toLowerCase() == 'prey') {
			switch (curStep)
			{
				...
				case 1543:
					...
					funnitext.text = "Seems like that bucket of bolts had to lay off the nitro this time around!";
					funnitext.y += 300;
	...
	override function beatHit()
	{
		...
		if (SONG.notes[Math.floor(curStep / 16)] != null)
		{
			...
			if (SONG.notes[Math.floor(curStep / 16)].mustHitSection || dad.animation.curAnim.name == 'idle')
				dad.dance();
		}
