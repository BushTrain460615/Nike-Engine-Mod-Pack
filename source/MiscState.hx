...
{
	var textMenuItems:Array<String> = ['1', '2', '3'];
	/*
	...
	*/
	...
	var money3:Alphabet = new Alphabet(0, 0, 'Week Progression', false, false);
		...
		if (controls.ACCEPT) {
			...
			if (money3.ID == curSelected) {
				if (Settings.WeekProgression) {
					trace('changed to false!');
					checkbox3.animation.play('unselected');
					Settings.WeekProgression = false;
					Settings.SettingsSave();
				}
				else {
					trace('changed to true!');
					checkbox3.animation.play('selecting animation');
					Settings.WeekProgression = true;
					Settings.SettingsSave();
				}
			}
		}
