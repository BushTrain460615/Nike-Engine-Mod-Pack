...
class ControlsState extends MusicBeatSubstate
{
	...
	var Text:Alphabet = new Alphabet(0, 0, "Primary keybinds", true, false);
	...
	override function update(elapsed:Float)
	{
		...
		*/
		...
		if (FlxG.keys.justPressed.UP && !CanPress || FlxG.keys.justPressed.W && !CanPress)
			curSelected -= 1;
	
		if (FlxG.keys.justPressed.DOWN && !CanPress || FlxG.keys.justPressed.S && !CanPress)
			curSelected += 1;
